...
class StationInfo extends React.PureComponent {
  ...
  render() {
    ...
    console.log('项目详情新增字段', toJS(projectDetail));
    return (
      <div className="_station-info">
        <List className="info-list">
          ...
          <Item extra={`${projectDetail.totalCostOfRawMaterials || '-'}`}>
            原材料成本
          </Item>
          <Item extra={`${projectDetail.otherCosts || '-'}`}>其他成本</Item>
          <Item extra={`${projectDetail.otherAccessoryCosts || '-'}`}>
            其他配件成本
          </Item>
          ...
          <Item extra={`${projectDetail.installationCost || '-'}`}>
            安装成本
          </Item>
          <Item extra={`${projectDetail.estimatedElectricityPrice || '-'}`}>
            预计上网电价
          </Item>
          <Accordion className="station-accordion">
